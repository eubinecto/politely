...
def test_honorify_nieun_dae(styler):
    ...
    """
    sent = "밥먹고 누우면 안 된대."
    assert "밥먹고 누우면 안 된대.", styler(sent, 1)
    assert "밥먹고 누우면 안 된대요.", styler(sent, 2)
...
def test_honorify_yi_ya(styler):
    ...
    assert "그 일은 제 담당이에요.", styler(sent, 2)
...
def test_honorify_eo_q(styler):
    ...
    # 했어?
    sent = "어제 공부는 마무리 했어?"
    assert "어제 공부는 마무리 했어?", styler(sent, 1)
    assert "어제 공부는 마무리 했어요?", styler(sent, 2)
...
def test_honorify_ddae_q(styler):
    ...
    assert "순서를 바꾸는건 어떻습니까?", styler(sent, 3)
    sent = "순서를 바꾸는건 어때요?"
    assert "순서를 바꾸는건 어때?", styler(sent, 1)
    assert "순서를 바꾸는건 어때요?", styler(sent, 2)
    assert "순서를 바꾸는건 어떻습니까?", styler(sent, 3)
...
def test_conjugate_ah_2(styler):
    ...
    """
    sent = "자, 떠나자. 동해바다로."
    assert "자, 떠나자. 동해바다로.", styler(sent, 1)
    assert "자, 떠나요. 동해바다로.", styler(sent, 2)
...
def test_conjugate_ah_3(styler):
    ...
    """
    sent = "자, 떠나요. 동해바다로."
    assert "자, 떠나. 동해바다로.", styler(sent, 1)
    assert "자, 떠나요. 동해바다로.", styler(sent, 2)
...
def test_conjugate_u(styler):
    ...
    """
    sent = "이 포스팅 퍼갈게."
    assert "이 포스팅 퍼갈게.", styler(sent, 1)
    assert "이 포스팅 퍼갈게요.", styler(sent, 2)
...
def test_conjugate_o(styler):
    ...
    """
    sent = "오늘 제주도로 여행왔어."
    assert "오늘 제주도로 여행왔어.", styler(sent, 1)
    assert "오늘 제주도로 여행왔어요.", styler(sent, 2)
...
def test_conjugate_yue(styler):
    ...
    """
    sent = "나는 그리하지 아니하였다."
    assert "나는 그리하지 아니하였다.", styler(sent, 1)
    assert "저는 그리하지 아니했어요.", styler(sent, 2)
    assert "저는 그리하지 아니했습니다.", styler(sent, 3)
    sent = "저는 그리하지 아니하였어요."
    assert "나는 그리하지 아니했어.", styler(sent, 1)
    assert "저는 그리하지 아니하였어요.", styler(sent, 2)
    assert "저는 그리하지 아니했습니다.", styler(sent, 3)
...
def test_contextual_2(styler):
    ...
    """
    sent = "하지말라고 일렀다."
    assert "하지말라고 일렀다.", styler(sent, 1)
    assert "하지말라고 일렀어요.", styler(sent, 2)
    assert "하지말라고 일렀습니다.", styler(sent, 3)
    sent = "드디어 정상에 이르렀다."
